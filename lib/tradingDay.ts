 ...
 *
 * @param closedDays AC-GL-4：該市場休市的平日（美國勞動節之類），一併扣掉。
 *   由 fetch-global.mjs 從 Yahoo 序列的缺口推出，不必維護假日表。
 */
export function businessDaysBehind(
  date: string | null, base: string | null, closedDays: string[] = [],
): number | null {
  if (!date || !base) return null
  const a = Date.parse(date + 'T00:00:00Z')
  const b = Date.parse(base + 'T00:00:00Z')
